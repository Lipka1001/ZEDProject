...
5. [Wykresy rozk³adow liczby atomow i elektronow](#wykresy-rozkladow-liczby-atomow-i-elektronow)
...
Raport dotyczy analizy danych krystalograficznych pochodz¹cych z bazy Protein Data Bank (PDB). 

Du¿a liczba atrybutów w zbiorze mog³a spowodowaæ nieczytelnoœæ korelacji, dlatego nale¿a³o zawêziæ zbiór do wybranych kolumn.

Bardzo wa¿ny okaza³ siê wybór atrybutów wykorzystywanych w regresji i klasyfikacji - zale¿a³a od niego poprawnoœæ predykcji.

Jedn¹ z obserwacji jest fakt, ¿e pozycje w rankingu klas z najwiêksz¹ niezgodnoœci¹ liczby atomów s¹ podobne do pozycji w rankingu klas z najwiêksz¹ niezgodnoœci¹ liczby elektronów.

Mo¿na zauwa¿yæ, ¿e wykorzystanie uczenia maszynowego mo¿e pomóc biologom w stwierdzeniu jakie cz¹steczki mog¹ siê kryæ w niewymodelowanych fragmentach map intensywnoœci.

Podstawow¹ trudnoœci¹ podczas wykonywania projektu by³a nieznajomoœæ dziedziny, której dotyczy³y dane. Osoba posiadaj¹ca odpowiedni¹ wiedzê biologiczn¹ z pewnoœci¹ wyci¹gnê³aby z analizy wiêcej wniosków i zauwa¿y³a dodatkowe zale¿nosci.
...
##Statystyki
Analizowany zbór danych zawiera `r nrow(prot_data2)` wierszy i `r ncol(prot_data2)` kolumn. Poni¿ej zamieszczono najwa¿niejsze statystyki. Statystyki dotycz¹ tylko danych typu numeric lub integer.
```{r stats}
...
##Korelacja
Analizowany zbiór danych posiada du¿¹ liczbê atrybutów. Korelacja miêdzy wszystkimi atrybutami by³aby nieczytelna graficznie i trudna do analizy. Dlatego korelacja zosta³a przedstawiona tylko dla kolumn rozpoczynaj¹cych siê od "part_0". Kolumny te reprezentuj¹ atrybuty ró¿nych poziomów odciêcia intensywnoœci. 
```{r Korelacja}
...
##Top 50 klas
Poni¿sza tabela przedstawia 50 najczêœciej wystêpuj¹cych nazw ligandu (res_name) w zbiorze danych.
```{r Top50}
prettyTable(p)
```

##Wykresy rozkladow liczby atomow i elektronow
###a) rozk³ad liczby atomów
```{r rozk³ad_atomow}
histogram1 <- ggplot(prot_data3, aes(x=local_res_atom_non_h_count)) + geom_histogram(binwidth=1, fill="white", colour="black")
histogram1
```

###b) rozk³ad liczby elektronów
```{r rozk³ad_elektronow}
...
Dla danych pogrupowanych po atrybucie res_name obliczono œredni¹ niezgodnoœæ liczby atomów i elektronów. Klasy z najwiêksz¹ niezgodnoœci¹ przedstawiono w tabeli poni¿ej.

###a) klasy z najwiêksz¹ nizgodnoœci¹ liczby atomów
```{r diff_a}
...
###b) klasy z najwieksz¹ niezgodnoœcia liczby elektronów
```{r diff_e}
...
Poni¿sze wykresy przedstawiaj¹ rozk³ad wartoœci kolumn o nazwach zaczynaj¹cych siê od "part_01". Na wykresach zaznaczono œredni¹ wartoœæ.
```{r part_01}
...
##Regresja
W tej sekcji sprawdzono, za pomoca regresji liniowej, czy na podstawie wartoœci innych kolumn mo¿na przewidzieæ liczbê elektronów i atomów. Dla regresji wybrane zosta³y nastêpuj¹ce atrybuty: local_volume, local_electrons, local_mean, local_std, local_min, local_max, local_skewness, resolution.
```{r Regresja}
...
Dla atomów wartoœæ miary RMSE wynos³a `r RegAtom$results$RMSE`, a wartoœæ miary R^2 `r RegAtom$results$Rsquared`.
Dla elektronów wartoœæ miary RMSE wynios³a `r RegElectron$results$RMSE`, a wartoœæ miary R^2 `r RegElectron$results$Rsquared`.

##Klasyfikator
W tej sekcji stworzono klasyfikator, który przewiduje wartoœæ res_name na podstawie wartoœci w innych kolumnach. Jako metodê klasyfikacji wybrano Random Forest. Do klasyfikacji wybrano atrybuty: FoFc_mean, FoFc_std, FoFc_square_std, FoFc_min, FoFc_max, resolution.
```{r Klasyfikator}
...
Miary jakoœci klasyfikatora wynios³y: Accuracy: `r overall_table[1]`, Kappa: `r overall_table[2]`.
